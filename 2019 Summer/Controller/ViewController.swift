...
extension ViewController: SearchInputViewDelegate {
    ...
    func selectedAnnotation(withMapItem mapItem: MKMapItem) {
        mapScreen.annotations.forEach { (annotation) in
...
extension ViewController {
    ...
    func zoomToFit(selectedAnnotation: MKAnnotation?) {
        if mapScreen.annotations.count == 0 {
            return
        }
        ...
        if let selectedAnnotation = selectedAnnotation {
            for annotation in mapScreen.annotations {
            ...
            region = mapScreen.regionThatFits(region)
    ...
    func generatePolyline(forDestinationMapItem destinationMapItem: MKMapItem) {
        ...
        directions.calculate { (response, error) in
            ...
            self.mapScreen.addOverlay(polyline)
        }
    }
    ...
    func centerMapOnUserLocation(shouldLoadAnnotations: Bool) {
        ...
        mapScreen.setRegion(coordinateRegion, animated: true)
    ...
    func removeAnnotations() {
        mapScreen.annotations.forEach { (annotation) in
            if let annotation = annotation as? MKPointAnnotation {
                mapScreen.removeAnnotation(annotation)
            }
        }
    }
    ...
    func loadAnnotations(withSearchQuery query: String) {
        ...
        searchBy(naturalLanguageQuery: query, region: region, coordinates: coordinate) { (response, error) in
            ...
            response.mapItems.forEach({ (mapItem) in
                ...
                self.mapScreen.addAnnotation(annotation)
            })
